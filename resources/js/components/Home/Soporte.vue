<template>
    <div class="container py-5 mt-5">
        <section class="py-5">
            <header class="section-header mt-5 mb-5 text-center">
                <h3>Soporte a usuarios</h3>
            </header>
            <div class="text_header">
                <h5>Responsable del servicio</h5>
                <div class="mb-4">
                    <span>
                        Funcionario de Departamento de Sistemas y TIC de UNISANGIL. Dirección Departamento de Sistemas y TIC.
                        Taller de mantenimiento.
                        Director, Coordinador de Sistemas y TIC sede Yopal.
                        Coordinador de Sistemas y Tic, sede Chiquinquirá
                    </span>
                </div>
                <div class="row">
                    <div class="content_contact col-md-6">
                        <h5>Contacto de Soporte</h5>
                        <p>
                            Ticket a través de la Mesa de TI, plataforma GLPI, Llamada <br>
                            •	Taller de mantenimiento, Área de redes, sede San Gil <br>
                            •	Teléfono 7245757  Extensión  230 – 220 <br>
                            •	Director, Coordinador de Sistemas y TIC sede Yopal  Correo electrónico <a href="mailto:yopal_tic@unisangil.edu.co">yopal_tic@unisangil.edu.co</a> <br>
                            •	Coordinador de Sistemas y Tic, sede Chiquinquirá, correo electrónico <a href="mailto:sistemaschiquinquira@unisangil.edu.co">sistemaschiquinquira@unisangil.edu.co</a>
                        </p>
                    </div>
                    <div class="content_suport col-md-6">
                        <h5>Horario de soporte</h5>
                        <p>
                            •	Lunes a viernes de 8:00 – 12:00 y  2:00 – 7:00 p.m. <br>
                            •	Sábado de 9:00 – 12:00 p.m.
                        </p>
                        <div class="text-center">
                            <img src="storage/img/services/soporte.png" alt="" class="img-fluid" width="250">
                        </div>
                    </div>
                </div>
                <div class="col-md-12 mt-5 text-center">
                    <h5 class="mb-0">SERVICIOS</h5>
                </div>
            </div>
            <div class="mt-0">
                <div class="accordion" id="accordionExample">
                    <div class="card">
                        <div class="card-header collapsed" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="false" aria-controls="collapseSix" id="headingOne">
                            <h2 class="btn btn-link mb-0">
                                •	Soporte a usuarios
                            </h2>
                        </div>

                        <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
                            <div class="card-body">
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Objetivo</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Brindar atención y solución a los incidentes técnicos de hardware y software presentados en los recursos tecnológicos que soportan los procesos misionales y reportados por los funcionarios de UNISANGIL.
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Descripción</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Servicio de asistencia técnica al hardware y software de los  equipos de cómputo que soportan los procesos misionales y de apoyo a la institución, mantenimiento preventivo y correctivo de las diferentes partes físicas de los equipos de escritorio y portátiles así como de impresoras, puntos de red y demás periféricos, reinstalación de equipos por daño o reclamaciones por garantía.
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Necesidad que Satisface</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Solución de incidentes de hardware y software de los equipos tecnológicos <br>
                                            Desarrollo de la gestión académica y administrativa <br>
                                            Desarrollo de la gestión de la investigación  y la extensión
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Usuario objetivo</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Comunidad universitaria
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header collapsed" type="button" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseSix" id="headingTwo">
                            <h2 class="btn btn-link mb-0">
                                •	Préstamo de laboratorios
                            </h2>
                        </div>
                        <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample">
                            <div class="card-body">
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Objetivo</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Ofrecer a la comunidad universitaria el préstamo de los laboratorios de cómputo para el desarrollo de las diferentes actividades académicas y administrativas.
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Descripción</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Asignación y prestamos de los recursos de laboratorios de acuerdo a los requerimientos recibidos. <br>
                                            Servicio de asistencia técnica al hardware y software de los equipos de cómputo instalados en los laboratorios, alistamiento para el desarrollo de las actividades académicas del semestre.
                                            Instalación de software específico para las actividades programadas.
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Necesidad que Satisface</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Solución de incidentes de hardware y software de los equipos tecnológicos <br>
                                            Desarrollo de la gestión académica y administrativa <br>
                                            Desarrollo de la gestión de la investigación  y la extensión
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Usuario objetivo</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Comunidad universitaria
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header collapsed" type="button" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseSix" id="headingThree">
                            <h2 class="btn btn-link mb-0">
                                •	Préstamo de portátiles
                            </h2>
                        </div>
                        <div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
                            <div class="card-body">
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Objetivo</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Ofrecer a la comunidad universitaria el préstamo de equipos portátiles, para el desarrollo de las diferentes actividades académicas y administrativas.
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Descripción</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Asignación y prestamos de equipos portátiles de acuerdo a los requerimientos recibidos.
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Necesidad que Satisface</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Solución de incidentes de hardware y software de los equipos tecnológicos <br>
                                            Desarrollo de la gestión académica y administrativa <br>
                                            Desarrollo de la gestión de la investigación  y la extensión
                                        </p>
                                    </div>
                                </div>
                                <div class="content_body row mb-2">
                                    <div class="col-12 col-sm-4 col-md-3 text-center my-auto">
                                        <h5>
                                            <strong>Usuario objetivo</strong>
                                        </h5>
                                    </div>
                                    <div class="col-12 col-sm-8 col-md-9">
                                        <p class="mb-2">
                                            Personal Administrativo
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>
</template>
<style lang="scss" scoped>
.btn-link{
    font-size: 18px;
    font-weight: 600;
    &:hover {
        color: #413e66;
        text-decoration: none;
    }
}
.card-header{
    &:hover {
        background-color: #cac9dd;
        text-decoration: none;
    }
}
.content_body{
    border-bottom: 1px solid;
    h5{
        color: #696592;
        font-weight: 400;
        font-size: 16px;
        font-style: italic;
    }
}
.text_header{
    margin-bottom: 50px;
    h5{
        color: #696592;
        font-weight: 600;
        font-size: 22px;
        font-style: italic;
    }
    p{
        margin: 0 0 10px 0;
    }
    .content_contact{
        border-left: 1px solid;
        border-right: 1px solid;
    }
    .content_suport{
        border-right: 1px solid;
    }
}
.section-header{
    text-transform: uppercase;
}
</style>
